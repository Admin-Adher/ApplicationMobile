    ...
    .map((r, i) => {
      ...
      return `<div style="margin-bottom:16px;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden">
        <div style="background:#f8fafc;padding:8px 12px;font-size:11px;font-weight:700;color:#1e293b;border-bottom:1px solid #e2e8f0">
          ...
          <span style="color:#94a3b8;font-weight:400;margin-left:8px">— ${cos.map(c => escHtml(c)).join(', ') || copy.noValue} · ${copy.buildingShort} ${escHtml(r.building || '?')}</span>
        </div>
...
function buildReserveRow(r: PdfReserveItem, idx: number, lang: ReportLanguage = 'fr'): string {
  ...
  return `<tr style="background:${bg}">
    ...
    <td style="padding:7px 10px">${escapeHtml(r.company) || copy.noValue}</td>
    <td style="padding:7px 10px">${escapeHtml(r.level) || copy.noValue}</td>
    <td style="padding:7px 10px"><span style="color:${statusColor};font-weight:600">${statusLabel(r.status, lang)}</span></td>
    <td style="padding:7px 10px"><span style="color:${priorityColor};font-weight:600">${priorityLabel(r.priority, lang)}</span></td>
    <td style="padding:7px 10px">${escapeHtml(r.deadline) || copy.noValue}</td>
  </tr>`;
}
...
function buildTableHeader(bgColor = '#003082', lang: ReportLanguage = 'fr'): string {
  ...
  return `<thead><tr style="background:${bgColor}">
    ...
    <th style="color:#fff;padding:8px 10px;text-align:left;font-size:10px;font-weight:700">${copy.title}</th>
    ...
    const orphanRows = orphanReserves
      .map((r, i) => {
        ...
        return `<tr style="background:${bg}">
          ...
          <td style="padding:7px 10px;font-weight:600">${escapeHtml(r.title)}</td>
          <td style="padding:7px 10px">${escapeHtml(r.company) || copy.noValue}</td>
          <td style="padding:7px 10px">${escapeHtml(r.building) || copy.noValue}</td>
          <td style="padding:7px 10px">${escapeHtml(r.level) || copy.noValue}</td>
          <td style="padding:7px 10px"><span style="color:${statusColor};font-weight:600">${statusLabel(r.status, lang)}</span></td>
          <td style="padding:7px 10px"><span style="color:${priorityColor};font-weight:600">${priorityLabel(r.priority, lang)}</span></td>
          <td style="padding:7px 10px">${escapeHtml(r.deadline) || copy.noValue}</td>
