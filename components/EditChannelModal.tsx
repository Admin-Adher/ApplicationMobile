...
interface Props {
  ...
  onClose: () => void;
  onSave: (id: string, updates: { name: string; description: string; icon: string; color: string }) => void;
}
...
export default function EditChannelModal({ visible, channel, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    if (channel) {
      ...
      setSelectedIcon(channel.icon ?? 'chatbubbles');
      setSelectedColor(channel.color ?? '#0A84FF');
    }
  }, [channel]);
  ...
  function handleSave() {
    ...
    if (channel) {
      onSave(channel.id, { name: trimmed, description: description.trim(), icon: selectedIcon, color: selectedColor });
    }
    onClose();
  }
