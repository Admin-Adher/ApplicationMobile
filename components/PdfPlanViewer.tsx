...
function rememberResolvedPlanUri(planUri: string, resolvedUri: string, fromCache: boolean) {
  ...
  while (resolvedPlanUriCache.size > PLAN_DATA_URL_CACHE_LIMIT) {
    const oldest = resolvedPlanUriCache.keys().next().value;
    if (!oldest) break;
    resolvedPlanUriCache.delete(oldest);
  }
}

function getRememberedResolvedPlanUri(planUri: string) {
  const cached = resolvedPlanUriCache.get(planUri);
...
function getPlanDrawingSignature(drawings: PlanDrawing[] | undefined | null): string {
  const normalized = (drawings ?? []).map(d => ({
    id: d.id,
    tool: d.tool,
    page: d.page ?? 1,
    color: d.color,
...
#inner{position:absolute;top:0;left:0;transform-origin:0 0;will-change:transform;backface-visibility:hidden;transform:translate3d(0,0,0);}
#pdf-canvas{display:block;box-shadow:0 4px 24px rgba(0,0,0,0.5);backface-visibility:hidden;}
...
var transformRaf=null,interactionTimer=null,interactionActive=false;
...
function setInteracting(active){
  ...
  if(active)container.classList.add('is-interacting');
  else container.classList.remove('is-interacting');
}

function markInteracting(){
  if(!interactionActive)setInteracting(true);
...
function getPinVisualScale(mult,rawSize){
  ...
  var count=Math.max((pinsData&&pinsData.length)||0,(ghostPinsData&&ghostPinsData.length)||0);
  var minReadableSize=count>=160?14.5:15.5;
  var maxReadableSize=count>=160?18:19;
  var targetScreenSize=Math.max(minReadableSize,Math.min(maxReadableSize,baseSize*densityScale*(mult||1)));
  var scale=targetScreenSize/(baseSize*safeZoom);
  var minScale=minReadableSize/(baseSize*safeZoom);
  return Math.max(minScale,Math.min(1.12,scale));
}
...
function getPinLabelFontSize(num,rawSize){
  var digits=String(num||'').length;
  var ratio=digits>=3?0.54:(digits===2?0.6:0.66);
  return Math.max(8,Math.round((rawSize||PIN_SIZE)*ratio));
}
...
function renderGhostPins(){
  ...
  ghostPinsData.forEach(function(pin){
    ...
    span.style.fontSize=getPinLabelFontSize(pin.num,sz)+'px';
...
var MAX_ADAPTIVE_RENDER_QUALITY=4;
...
function scheduleAdaptiveRerender(){
  ...
  rerenderTimer=setTimeout(function(){
    if(interactionActive||isPinching||panning){scheduleAdaptiveRerender();return;}
    var neededQ=Math.min(MAX_ADAPTIVE_RENDER_QUALITY,Math.max(SHARP_RENDER_QUALITY,Math.ceil(zoom*1.1)));
    if(zoom>1.7&&neededQ>RENDER_QUALITY&&!isRerendering){
      RENDER_QUALITY=neededQ;
...
if(IS_IMAGE){
  ...
  img.onerror=function(){
    loading.style.display='none';
    errMsg.innerHTML=COPY.loadImageError+'<br>'+COPY.checkConnection;
    errMsg.style.display='flex';
    post({type:'planError',error:'image_load_failed'});
  };
  img.src=PLAN_URI;
}else{
  loadPdfjs().then(function(pdfjsLib){
    ...
    loading.style.display='none';
    var errorCode=String((err&&err.name)||'pdf_load_failed').replace(/[^a-zA-Z0-9_. -]/g,'').slice(0,80)||'pdf_load_failed';
    errMsg.innerHTML=COPY.loadPdfError+'<br>'+COPY.checkConnection+'<br><span style="color:#94A3B8;font-size:11px;margin-top:8px;display:block">'+errorCode+'</span>';
    errMsg.style.display='flex';
    post({type:'planError',error:errorCode});
  });
}
...
container.addEventListener('touchend',function(e){
  ...
  panning=false;isDragging=false;endInteractionSoon();
},{passive:false});
...
window.setAnnotations=function(anns,force){
  ...
  draws=anns;
  if(incomingSignature===lastLocalAnnotationSignature)hasLocalAnnotationChanges=false;
  renderAnns();postAnnotationState();
};
...
const MobileViewer = forwardRef<PdfPlanViewerHandle, PdfPlanViewerProps>(function MobileViewerInner({
  ...
  const activeFallbackKeyRef = useRef(fallbackKey);
  const remoteFallbackInFlightRef = useRef<string | null>(null);
  activeFallbackKeyRef.current = fallbackKey;
  ...
  const [resolvedUri, setResolvedUri] = useState<string>(
    rememberedResolvedUri?.resolvedUri ?? ((isLocalUri || isRemoteUri) ? '' : planUri),
  );
  const [uriLoading, setUriLoading] = useState((isLocalUri || isRemoteUri) && !rememberedResolvedUri);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [fromCache, setFromCache] = useState(rememberedResolvedUri?.fromCache ?? false);
  ...
  useEffect(() => {
    remoteFallbackInFlightRef.current = null;
    setInlineFallback(current => current?.key === fallbackKey ? current : null);
  }, [fallbackKey]);
  ...
  useEffect(() => {
    ...
    const alreadyLoadedSources = getLoadedBundledPdfJsSources();
    if (alreadyLoadedSources) {
      setPdfJsSource(alreadyLoadedSources.moduleSource);
      setPdfJsWorkerSource(alreadyLoadedSources.workerSource);
      setPdfJsLoading(false);
      return () => { cancelled = true; };
    }
  ...
  useEffect(() => {
    ...
    if (remembered) {
      ...
      setUriLoading(false);
      setLoadError(null);
      setFromCache(remembered.fromCache);
    ...
    async function loadRemote(): Promise<void> {
      // A local copy stays first. On a miss, display the authorized short-lived
      // URL immediately and let the offline download continue independently.
      try {
        const { uri: displayUri, fromCache: wasCached } = await getPlanUriForDisplay(planUri);
        if (cancelled) return;
        if (displayUri.startsWith('http://') || displayUri.startsWith('https://')) {
          setResolvedUri(displayUri);
          setFromCache(false);
          setUriLoading(false);
          setLoadError(null);
          setOfflineUnavailable(false);
        ...
        if (canUseDirectLocalUri(displayUri) && forcedInlineLocalUri !== displayUri) {
          rememberResolvedPlanUri(planUri, displayUri, wasCached);
          setResolvedUri(displayUri);
        ...
        const dataUrl = await inlineFromLocalFile(displayUri);
        if (cancelled) return;
        if (dataUrl) {
          rememberResolvedPlanUri(planUri, dataUrl, wasCached);
  ...
  const injectAnnotations = useCallback((force = false) => {
    ...
    inject(`window.setAnnotations(${serializedAnnotations},${force ? 'true' : 'false'});`);
    if (externalAnnotationSignature === localAnnotationSignatureRef.current) {
      hasPendingLocalAnnotationsRef.current = false;
      localAnnotationCountRef.current = externalAnnotationCount;
    }
  }, [inject, serializedAnnotations, externalAnnotationSignature, externalAnnotationCount]);
    ...
    try {
      if (msg.type === 'tap') {
        ...
        if (
          ...
          && !forcedInlineLocalUri
          && remoteFallbackInFlightRef.current !== fallbackKey
        ) {
          const attemptKey = fallbackKey;
          remoteFallbackInFlightRef.current = attemptKey;
          setUriLoading(true);
          setLoadError(null);
          setOfflineUnavailable(false);
          void ensurePlanCached(planUri)
            .then(({ localUri }) => {
              if (activeFallbackKeyRef.current !== attemptKey) return;
              setInlineFallback({ key: attemptKey, localUri });
            })
            .catch((error: unknown) => {
              if (activeFallbackKeyRef.current !== attemptKey) return;
              setUriLoading(false);
              setOfflineUnavailable(!isOnline);
              setLoadError(error instanceof Error ? error.message : t('pdfPlanViewer.planLoadError'));
            })
            .finally(() => {
              if (remoteFallbackInFlightRef.current === attemptKey) {
                remoteFallbackInFlightRef.current = null;
              }
            });
          return;
            ...
            {cw > 0 && pinsOnPage.map(r => {
              ...
              return (
                <div
                  key={r.id}
                  data-pin
                  onMouseDown={(e: any) => onPinMouseDown(e, r)}
                  onClick={(e: any) => {
                    e.stopPropagation();
                    if (!pinJustMovedRef.current) onReserveSelect(r);
                  }}
                  style={{
                    position: 'absolute',
                    left: (r.planX! / 100) * cw - sz / 2,
