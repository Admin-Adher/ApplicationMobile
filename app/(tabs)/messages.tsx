    ...
    return (
      <View style={{ marginBottom: 4 }}>
        ...
        {visibleCompanyChannels.length > 0 ? (
          <View style={styles.channelGroup}>
            {visibleCompanyChannels.map((ch, i) => renderCompanyChannelRow(ch, i > 0))}
          </View>
        ) : (
          <View style={styles.emptySection}>
                    ...
                    <View key={r.message.id}>
                      ...
                      >
                        <View style={[styles.globalResultIcon, { backgroundColor: r.channel.color + '20' }]}>
                          <Ionicons name={r.channel.icon as any} size={16} color={r.channel.color} />
                        </View>
                        <View style={{ flex: 1 }}>
                          <View style={styles.globalResultTop}>
                            <Text style={[styles.globalResultChannel, { color: r.channel.color }]} numberOfLines={1}>
                              {r.channel.name}
                            </Text>
                            <Text style={styles.globalResultTime}>
                              {r.message.timestamp.split(' ')[1] ?? ''}
                            </Text>
                          </View>
                          <Text style={styles.globalResultSender}>{r.message.sender}</Text>
                          <Text style={styles.globalResultContent} numberOfLines={2}>{r.message.content}</Text>
                        </View>
                        <Ionicons name="chevron-forward" size={14} color={C.textMuted} />
                      </TouchableOpacity>
                    </View>
...
const styles = StyleSheet.create({
  ...
  channelPreview: { fontSize: 13, fontFamily: 'Inter_400Regular', color: C.textSub, flex: 1, marginRight: 6 },
  ...
  emptySection: { backgroundColor: C.surface, borderRadius: 14, borderWidth: 1, borderColor: C.border, padding: 20, alignItems: 'center', marginBottom: 20 },
  emptySectionText: { fontSize: 13, fontFamily: 'Inter_400Regular', color: C.textMuted },
  globalResultItem: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12 },
  globalResultIcon: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
  globalResultTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  globalResultChannel: { fontSize: 11, fontFamily: 'Inter_600SemiBold', textTransform: 'uppercase', letterSpacing: 0.4, flex: 1 },
  globalResultTime: { fontSize: 10, fontFamily: 'Inter_400Regular', color: C.textMuted },
  globalResultSender: { fontSize: 12, fontFamily: 'Inter_600SemiBold', color: C.text, marginTop: 1 },
  globalResultContent: { fontSize: 12, fontFamily: 'Inter_400Regular', color: C.textSub, marginTop: 2, lineHeight: 17 },
  empty: { alignItems: 'center', paddingTop: 40, gap: 10 },
  emptyText: { fontSize: 14, fontFamily: 'Inter_400Regular', color: C.textMuted },
  quickActions: {
    backgroundColor: C.surface, borderRadius: 14, borderWidth: 1,
    borderColor: C.border, overflow: 'hidden', marginBottom: 20,
  },
  quickBtn: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 16 },
  quickBtnIcon: { width: 36, height: 36, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  quickBtnText: { flex: 1, fontSize: 14, fontFamily: 'Inter_500Medium', color: C.text },
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'flex-end' },
  sheet: { backgroundColor: C.surface, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 36 },
  ...
  sheetSub: { fontSize: 12, fontFamily: 'Inter_400Regular', color: C.textMuted },
  sheetDivider: { height: 1, backgroundColor: C.border, marginBottom: 12 },
  sheetBtn: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12 },
  sheetBtnIcon: { width: 40, height: 40, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  sheetBtnLabel: { fontSize: 15, fontFamily: 'Inter_600SemiBold', color: C.text },
