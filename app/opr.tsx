    ...
    ${leveedWithPhotos.map(({ item, reserve }) => {
      ...
      return `<div style="margin-bottom:20px;page-break-inside:avoid">
        ...
        <div style="display:flex;gap:16px;flex-wrap:wrap">
          ${photos.defect ? `<div style="text-align:center"><div style="position:relative;display:inline-block;max-width:100%;"><img src="${photos.defect.src}" style="width:auto;height:auto;max-width:200px;max-height:240px;background:#FFF5F5;border-radius:8px;border:2px solid #FCA5A5;display:block" />${buildPhotoAnnotationsOverlayHtml(photos.defect.annotations)}</div><div style="font-size:10px;color:#DC2626;font-weight:700;margin-top:4px">${escapeHtml(copy.initialFinding)}</div></div>` : ''}
          ${photos.resolution ? `<div style="text-align:center"><div style="position:relative;display:inline-block;max-width:100%;"><img src="${photos.resolution.src}" style="width:auto;height:auto;max-width:200px;max-height:240px;background:#F0FFF4;border-radius:8px;border:2px solid #6EE7B7;display:block" />${buildPhotoAnnotationsOverlayHtml(photos.resolution.annotations)}</div><div style="font-size:10px;color:#059669;font-weight:700;margin-top:4px">${escapeHtml(copy.closedFinding)}</div></div>` : ''}
        </div>
      </div>`;
    }).join('')}
  ` : '';
