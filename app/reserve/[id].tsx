...
type ReservePdfLanguage = 'fr' | 'en' | 'es';

interface ReservePdfCopy {
  lang: ReservePdfLanguage;
  titlePrefix: string;
  generatedOn: string;
  createdOn: string;
  closedOn: string;
  ...
  detail: string;
  signatures: string;
  liftSignature: string;
  signer: string;
  liftAcknowledgedOn: string;
  manager: string;
  contractor: string;
  confidential: string;
  generatedBy: string;
...
const RESERVE_PDF_COPY: Record<ReservePdfLanguage, ReservePdfCopy> = {
  fr: {
    lang: 'fr',
    titlePrefix: 'Fiche de réserve',
    ...
    noPlan: 'Aucun plan associé',
    planPin: 'Pastille de localisation',
    photos: 'Photos',
    of: 'sur',
    defect: 'Constat',
    resolved: 'Levée',
    ...
    signer: 'Signataire',
    liftAcknowledgedOn: 'Levée reconnue le',
    manager: 'Conducteur de travaux',
    contractor: 'Entreprise',
    confidential: 'Document confidentiel',
    generatedBy: 'Fiche réserve générée par BuildTrack',
    noValue: 'N/A',
  },
  en: {
    lang: 'en',
    titlePrefix: 'Issue sheet',
    ...
    signer: 'Signer',
    liftAcknowledgedOn: 'Closure acknowledged on',
    manager: 'Construction manager',
    contractor: 'Company',
    confidential: 'Confidential document',
    generatedBy: 'Issue sheet generated by BuildTrack',
    noValue: 'N/A',
  },
  es: {
    lang: 'es',
    titlePrefix: 'Ficha de incidencia',
    ...
    locationPlan: 'Plano de ubicación',
    noPlan: 'Sin plano asociado',
    planPin: 'Marcador de ubicación',
    photos: 'Fotos',
    of: 'de',
    defect: 'Incidencia',
    resolved: 'Cierre',
    ...
    detail: 'Detalle',
    signatures: 'Firmas',
    liftSignature: 'Firma de cierre',
    signer: 'Firmante',
    liftAcknowledgedOn: 'Cierre reconocido el',
    manager: 'Jefe de obra',
    contractor: 'Empresa',
    confidential: 'Documento confidencial',
    generatedBy: 'Ficha de incidencia generada por BuildTrack',
    noValue: 'N/A',
  },
};

function reservePdfLanguage(language?: string | null): ReservePdfLanguage {
...
function reservePdfLocale(language: ReservePdfLanguage): string {
  ...
  const photoRowHtml = photosToShow.length > 0
    ? `<div style="margin-top:10px">
        <div style="font-size:9px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:0.7px;margin-bottom:6px;padding-bottom:4px;border-bottom:1px solid #DDE4EE">
          ${copy.photos} (${photosToShow.length}${rawPhotos.length > MAX_PHOTOS ? ` ${copy.of} ${rawPhotos.length}` : ''})
        </div>
...
document.head.appendChild(s);
  ...
  const planSection = planData
    ? (imgSrc
      ...
      ? `<div style="position:relative;border-radius:8px;overflow:hidden;border:1.5px solid #DDE4EE">
          ...
          <div style="padding:3px 8px;background:rgba(0,0,0,0.45);font-size:9px;color:#fff;font-weight:600">
            ${planData.planName} — ${copy.planPin}
          </div>
        </div>`
    ...
    : `<div style="width:100%;height:140px;border-radius:8px;border:1.5px dashed #DDE4EE;background:#F9FAFB;
        ...
        <span style="font-size:10px;color:#9CA3AF">${copy.noPlan}</span>
      </div>`;
