-- Atomic unlink of reserves from a visit.
--
-- This is the inverse of link_reserves_to_visite:
-- - removes IDs from visites.reserve_ids
-- - clears reserves.visite_id only when it points to this visit
-- - keeps the reserve itself intact
-- - runs as the authenticated user so existing RLS/auth helper checks apply
