-- Repair visites.reserve_ids type for atomic visite/reserve RPC sync.
--
-- Older schema versions created public.visites.reserve_ids as jsonb.
-- The atomic RPCs use text[] operations (unnest, &&, ||), so a jsonb column
-- raises: [42804] COALESCE types jsonb and text[] cannot be matched.
