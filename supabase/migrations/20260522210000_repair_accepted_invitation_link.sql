...
--
-- The previous RPC only looked for invitations with status = 'pending'.
-- If signup accepted the invitation but profile creation/linking failed or was
-- interrupted, later logins could no longer repair the user because the
-- invitation was already 'accepted'. This version can repair both:
--   - pending, non-expired invitations
--   - already accepted invitations for the same email
