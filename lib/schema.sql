...
alter table public.reserves add column if not exists deleted_at timestamptz;
alter table public.reserves add column if not exists deleted_by text;
...
create index if not exists idx_reserves_not_deleted on public.reserves(chantier_id, plan_id) where deleted_at is null;
