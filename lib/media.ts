...
function cachedValue(ref: string): string | null {
  const cached = memoryCache.get(ref);
  if (!cached || cached.expiresAt - 60_000 <= Date.now()) return null;
  return cached.url;
}

function diskPath(assetId: string): string | null {
  if (Platform.OS === 'web' || !FileSystem.cacheDirectory) return null;
  return `${FileSystem.cacheDirectory}btmedia/${assetId}`;
}

async function existingDiskPath(assetId: string): Promise<string | null> {
  const path = diskPath(assetId);
  if (!path) return null;
  const info = await FileSystem.getInfoAsync(path).catch(() => null);
  return info?.exists ? path : null;
}

async function cacheOnDisk(assetId: string, url: string): Promise<string | null> {
  const path = diskPath(assetId);
  if (!path) return null;
  const existing = await existingDiskPath(assetId);
  if (existing) return existing;
  const directory = `${FileSystem.cacheDirectory}btmedia/`;
...
async function resolveBatch(refs: string[]): Promise<Map<string, ResolvedMedia>> {
  ...
  if (!token || token === SUPABASE_KEY) return resolved;
  ...
  for (const item of Array.isArray(body?.assets) ? body.assets : []) {
    ...
    memoryCache.set(entry.ref, entry);
    memoryCache.set(`btmedia://${entry.assetId}`, entry);
...
export async function resolveMediaRefs(refs: string[], options?: { cacheDisk?: boolean }): Promise<Record<string, string>> {
  ...
  for (const ref of refs) {
    ...
    if (assetId) {
      const disk = await existingDiskPath(assetId);
      if (disk) {
        output[ref] = disk;
        continue;
      }
    }
    const cached = cachedValue(ref);
    if (cached) output[ref] = cached;
    else missing.push(ref);
  }

  const fresh = await resolveBatch(missing);
  for (const ref of missing) {
    const item = fresh.get(ref) ?? memoryCache.get(ref);
    if (!item) continue;
    let url = item.url;
    if (options?.cacheDisk !== false && Platform.OS !== 'web') {
      url = await cacheOnDisk(item.assetId, item.url) ?? item.url;
    }
    output[ref] = url;
  }
  return output;
...
export async function resolveMediaRef(ref: string, options?: { cacheDisk?: boolean }): Promise<string | null> {
  ...
  if (existing) return existing;
  const work = resolveMediaRefs([ref], options).then(result => result[ref] ?? null).finally(() => pending.delete(ref));
  pending.set(ref, work);
  return work;
}
