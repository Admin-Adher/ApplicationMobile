...
- **BUG CRITIQUE (chargement infini à la première ouverture) — AuthContext.tsx + AppContext.tsx:** Lors de la première ouverture (session=null), le processus de seeding (création des utilisateurs démo) déclenchait des événements `SIGNED_OUT` *avant* que `INITIAL_SESSION` ne soit reçu. AppContext réagissait à ces `SIGNED_OUT` en effaçant toutes les données et en incrémentant `loadGenerationRef`, ce qui annulait chaque `loadAll()` successif. Une fois l'utilisateur connecté, le seeding en cours déclenchait un second `SIGNED_IN`, interrompant le `loadAll()` en cours (gen=3 → gen=4 → …). Fix en deux parties : (1) **`AuthContext.tsx`** exporte un `globalSeedingRef` module-level que tous les modules peuvent lire sans déclencher de re-render React ; il est synchronisé avec `isSeedingRef` sur chaque démarrage/arrêt du seeding. (2) **`AppContext.tsx`** importe ce flag et ajoute deux gardes dans son handler `onAuthStateChange` : Guard 1 — ignore tous les événements jusqu'à la réception de `INITIAL_SESSION` (les `SIGNED_OUT`/`SIGNED_IN` du seeding arrivent avant) ; Guard 2 — ignore les événements lorsque `globalSeedingRef.current === true` (protège contre les événements de seeding après `INITIAL_SESSION`). `INITIAL_SESSION` est désormais géré explicitement : s'il arrive avec une session, il déclenche `loadAll()` ; sans session, il dispatche `SET_LOADING: false` pour déverrouiller l'écran de connexion.
